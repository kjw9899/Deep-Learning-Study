...
class Trainer():
    def __init__(
        ...
        self.train_3d_loader, self.disc_motion_loader, self.valid_loader = data_loaders
        ...
        self.generator_ps = generator_ps
    ...
    def train(self):
        ...
        # nums = len(self.train_2d_loader) + len(self.train_3d_loader)
        nums = len(self.train_3d_loader)
        
        # bar = Bar(f'Epoch {self.epoch + 1}/{self.end_epoch}', fill='#', max=self.num_iters_per_epoch)
        bar = Bar(f'Epoch {self.epoch + 1}/{self.end_epoch}', fill='#', max=nums)
        ...
        for i in range(nums):
            ...
            for name, param in self.generator_ps.named_parameters():
                param.requires_grad_(False)
            
            with torch.no_grad():
                preds_ps, scores_ps, preds_Dm_ps = self.generator_ps(inp, is_train=True)
...
##            )
            ...
            save_target_3d = target_3d
            save_target_2d = target_2d
            # del save_target_3d['features']
            # del save_target_2d['features']
            
            preds_ps_3d ={
                'theta' : preds_ps[-1]['theta'][-1],
                'verts' : preds_ps[-1]['verts'][-1],
                'kp_2d' : preds_ps[-1]['kp_2d'][-1],
                'rotmat' : preds_ps[-1]['rotmat'][-1],
            }
            
            
            with open(f'MPSNet_outputs/3dpw_img/targets/targets_{cnt}.plk', 'wb') as f: # 3dpw, h36m, 3dhp
                pickle.dump(save_target_3d, f)
            
            # with open(f'MPSNet_outputs/targets_2d/target_2d_{cnt}.plk', 'wb') as f:
            #     pickle.dump(save_target_2d, f)
            
            with open(f'MPSNet_outputs/3dpw_img/preds/preds_{cnt}.plk', 'wb') as f:
                pickle.dump(preds_ps_3d, f)
            
            # with open(f'MPSNet_outputs/targets_2d/target_2d_{cnt}.plk', 'wb') as f:
            #     pickle.dump(save_target_2d, f)
            
            # with open(f'MPSNet_outputs/targets_3d/target_3d_{cnt}.plk', 'wb') as f:
            #     pickle.dump(save_target_3d, f)
            
            # with open(f'MPSNet_outputs/preds/preds_{cnt}.plk', 'wb') as f:
            #     pickle.dump(preds_ps, f)
            
            cnt+=1
            # exclude motion discriminator
            
            # preds[-1]['theta] = [batch, seqlen, 85]
            # target_3d['theta'] = [batch, seqlen, 85]
...
##            total_loss = gen_loss
            ...
            summary_string = f'({i + 1}/{nums}) | Total: {bar.elapsed_td} | ' \
    ...
    def validate(self):
        ...
        for i, target in enumerate(self.valid_loader):
            ...
            with torch.no_grad():
                ...
                preds, _, preds_Dm = self.generator(inp, J_regressor=J_regressor)
                preds_ps, _, preds_Dm = self.generator_ps(inp, J_regressor=J_regressor)#
                
                save_target_3d = target
                # del save_target_3d['features']
                
                preds_ps_3d ={
                    'theta' : preds_ps[-1]['theta'],
                    'verts' : preds_ps[-1]['verts'],
                    'kp_2d' : preds_ps[-1]['kp_2d'],
                    'rotmat' : preds_ps[-1]['rotmat'],
                }
                
                # with open(f'MPSNet_outputs/3dpw/val/targets/targets_{cnt}.plk', 'wb') as f: # 3dpw, h36m, 3dhp
                #     pickle.dump(save_target_3d, f)

                # # with open(f'MPSNet_outputs/targets_2d/target_2d_{cnt}.plk', 'wb') as f:
                # #     pickle.dump(save_target_2d, f)
                
                # with open(f'MPSNet_outputs/3dpw/val/preds/preds_{cnt}.plk', 'wb') as f:
                #     pickle.dump(preds_ps_3d, f)
                cnt += 1
