...
class Dataset3D(Dataset):
    ...
    def get_single_item(self, index):
        ...
        start = img_names[0].split('/')[-1]
        end = img_names[-1].split('/')[-1]
        # with open(f'MPSNet_outputs/instrancd_id_3d/inst_3d_{start}_{end}.plk', 'wb') as f:
        #         pickle.dump(img_names, f)
        
        repeat_num = 3    
        target = {
            'features': input, # [16, 2048]
            'theta': torch.from_numpy(theta_tensor).float()[self.mid_frame].repeat(repeat_num, 1), # camera, pose and shape, [3, 85]
            'kp_2d': torch.from_numpy(kp_2d_tensor).float()[self.mid_frame].repeat(repeat_num, 1, 1), # 2D keypoints transformed according to bbox cropping [3, 49, 3]
            ...
            'img_names': list(img_names)[self.mid_frame],
        }
